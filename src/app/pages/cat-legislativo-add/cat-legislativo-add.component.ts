...
@Component({
  selector: 'app-cat-legislativo-add',
  templateUrl: './cat-legislativo-add.component.html',
  styles: []
})
export class CatLegislativoAddComponent implements OnInit {
  ...
  nuevaCat(){
    if( this.frmPwd.valid ){
      ...
      this.catServices.addCatLegislativa(this.frmPwd.value)
      .subscribe( (resp: any) =>{ 
        console.log(resp);
        if( resp.ok){
          Swal.fire({
